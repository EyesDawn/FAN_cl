...
# 定义预测模型
class FANPredictor(nn.Module):
    def __init__(self, seq_len, pred_len, enc_in, freq_topk=10):
        super(FANPredictor, self).__init__()
        self.fan = FAN(seq_len=seq_len, pred_len=pred_len, enc_in=enc_in, freq_topk=freq_topk)
    ...
    def forward(self, x):
        ...
        # 反归一化得到最终预测结果
        output = self.fan.denormalize(output)
        return output

# 初始化模型
model = FANPredictor(seq_len=seq_len, pred_len=pred_len, enc_in=len(selected_features), freq_topk=freq_topk).to(device)
...
plt.tight_layout()
plt.savefig('fan_prediction_results.png')
plt.show()

# 保存模型
torch.save(model.state_dict(), 'fan_model.pth')
print("模型已保存到 fan_model.pth")